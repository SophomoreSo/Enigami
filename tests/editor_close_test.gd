...
func _ready() -> void:
	...
	await frames(10)
	check(not raid.editing and not raid.editor.visible, "it stays closed after 10 frames")
	...
	await click(raid.editor._close_rect().get_center())
	...
	check(sb.panel.visible, "the bench panel comes back")
	await key(KEY_TAB)
	await key(KEY_ESCAPE)
