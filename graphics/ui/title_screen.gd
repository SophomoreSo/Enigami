...
## The shapes drifting behind the title are components, loosely: they wear the
## colours of the part categories a board is built from.
const DRIFT_CATS := [Components.CAT_FORM, Components.CAT_FLOW,
	Components.CAT_BEHAVIOR, Components.CAT_TRIGGER]
...
func _ready() -> void:
	...
	for i in 26:
		_shapes.append({
			...
			"c": Style.category_color(DRIFT_CATS[rng.randi() % DRIFT_CATS.size()]),
		})
