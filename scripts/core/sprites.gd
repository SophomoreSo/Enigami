...
##
## See `assets/sprites/CREDITS.md`. Tile names follow
## `<base>_<idle|run|hit>_anim_f<n>`, except for a handful of monsters that
## ship a single unnamed loop as `<base>_anim_f<n>`.

const ATLAS_PATH := "res://assets/sprites/dungeon_atlas.png"
const SHADER_PATH := "res://assets/shaders/actor_sprite.gdshader"
...
## The player's silhouette: teal plate, which already matches the cyan the
## player was drawn in.
const PLAYER := "knight_m"

## Weapon held in the aim direction. Every weapon tile points up in the atlas.
const WEAPONS := {
