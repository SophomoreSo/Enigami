class_name DungeonAtlasFrames
extends RefCounted

## `tile_list_v1.7` from 0x72's CC0 "16x16 DungeonTileset II", verbatim: one
## line of `name x y width height` per tile in `assets/sprites/dungeon_atlas.png`.
## See `assets/sprites/CREDITS.md`.
##
## It lives in a script rather than beside the atlas as a text file because
## Godot only exports non-resource files that an export preset explicitly
## filters in — as a plain `.txt` the whole sprite set would load in the editor
## and silently vanish from an exported build.
