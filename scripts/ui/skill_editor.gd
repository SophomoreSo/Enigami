...
func _draw_drag() -> void:
	...
	draw_string(_font, r.position + Vector2(8, 23), String(def["glyph"]), HORIZONTAL_ALIGNMENT_LEFT, -1, 14, col)
...
func _draw_component(b: SkillBoard, origin: Vector2i) -> void:
	...
	draw_string(_font, center + Vector2(-CELL * 0.5, 4), String(def["glyph"]), HORIZONTAL_ALIGNMENT_CENTER, CELL, 16, glyph_col)
...
func _draw_palette() -> void:
	...
	for i in ids.size():
		...
		draw_string(_font, r.position + Vector2(8, 18), String(def["glyph"]), HORIZONTAL_ALIGNMENT_LEFT, -1, 13, c)
