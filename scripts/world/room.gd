...
var _extract_hold: float = 0.0
var _extract_active: bool = false
var _hazards: Array = []            ## spikes: {cell, damage}
	...
	if rng.randf() < 0.45:
		...
		for x in range(hx, hx + hlen):
			if not is_solid(x, H - 3) and is_solid(x, H - 2):
				_hazards.append(Vector2i(x, H - 3))
...
func _roll_enemies() -> Array:
	...
	for i in n:
		...
		if rng.randf() < 0.12 + 0.05 * float(danger):
			var keys := Enemy.MODIFIERS.keys()
			mod_id = String(keys[rng.randi() % keys.size()])
...
func _process(delta: float) -> void:
	...
	# Only the exit animates; the tiles are drawn once by the static layer.
	if not extraction.is_empty():
		queue_redraw()
...
func _update_extraction(delta: float) -> void:
	...
	if inside and reason == "" and Input.is_action_pressed("interact") and not player.input_locked:
		...
		if int(_extract_hold * 8.0) != int((_extract_hold - delta) * 8.0):
			Fx.burst(player.global_position, Color(0.6, 1.0, 0.8), 2, 60.0)
		var need := float(extraction.get("time", 2.5))
		extraction_progress.emit(clampf(_extract_hold / need, 0.0, 1.0), extraction)
		if _extract_hold >= need:
		...
		_extract_hold = maxf(0.0, _extract_hold - delta * 2.0)
...
## --- drawing ----------------------------------------------------------------
## Tiles, hazards and door frames never change while a room is loaded, so they
## live on their own layer that redraws only when the room is built.
class TileLayer extends Node2D:
	...
	func _draw() -> void:
		if room == null:
			return
		room.draw_static(self)

var _tiles: TileLayer

func _make_tile_layer() -> void:
...
func draw_static(c: CanvasItem) -> void:
	var bg := Color(0.075, 0.085, 0.11)
	c.draw_rect(Rect2(0, 0, W * CELL, H * CELL), bg)
	for x in range(0, W + 1, 2):
		c.draw_line(Vector2(x * CELL, 0), Vector2(x * CELL, H * CELL), Color(1, 1, 1, 0.022), 1.0)
	for y in range(0, H + 1, 2):
		c.draw_line(Vector2(0, y * CELL), Vector2(W * CELL, y * CELL), Color(1, 1, 1, 0.022), 1.0)
	...
	for y in H:
		for x in W:
			...
			if not is_solid(x, y - 1):
				c.draw_rect(Rect2(r.position, Vector2(CELL, 4)), tint.lightened(0.35))
			c.draw_rect(r, Color(0, 0, 0, 0.22), false, 1.0)

	for h in _hazards:
		var base: Vector2 = Vector2(h.x * CELL, (h.y + 1) * CELL)
		for i in 4:
			var x0: float = base.x + float(i) * CELL * 0.25
			c.draw_colored_polygon(PackedVector2Array([
				Vector2(x0, base.y), Vector2(x0 + CELL * 0.125, base.y - 14.0), Vector2(x0 + CELL * 0.25, base.y),
			]), Color(0.9, 0.35, 0.4))

	for dir in doors:
		var r := door_rect(int(dir))
		c.draw_rect(r, Color(0.4, 0.75, 0.95, 0.18))
		c.draw_rect(r, Color(0.45, 0.8, 1.0, 0.75), false, 2.0)
...
func _draw_extraction() -> void:
	var r := extraction_rect()
	var reason := extraction_blocked_reason()
	var col := Color(0.45, 0.95, 0.7) if reason == "" else Color(0.85, 0.5, 0.5)
	...
	var title := String(extraction.get("name", "EXIT"))
	draw_string(font, c + Vector2(-46, -60), title, HORIZONTAL_ALIGNMENT_CENTER, 92, 13, col)
	if _extract_hold > 0.0:
		var need := float(extraction.get("time", 2.5))
		draw_arc(c, 52.0, -PI * 0.5, -PI * 0.5 + TAU * clampf(_extract_hold / need, 0.0, 1.0), 32, Color(0.6, 1.0, 0.8), 4.0)

func _region_tint() -> Color:
	match int(data.get("region", 0)):
		1: return Color(0.20, 0.26, 0.34)
		2: return Color(0.28, 0.22, 0.32)
		3: return Color(0.32, 0.26, 0.20)
	return Color(0.18, 0.21, 0.27)
