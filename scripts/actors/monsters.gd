...
##
## `sprite` names the character in the shared atlas that draws this monster
## (see `Sprites`), chosen so the art reads as the AI: the only winged sprite
## is the only flyer, the legless caster is the turret. `color` is what a death
## burst throws, so it tracks the sprite's palette. `tint` multiplies the
## sprite when the art has to say something the pack does not — the Warden is
## an ice caster wearing a green ogre.

const DEFS := {
	"CRAWLER": {
		"name": "Crawler", "hp": 32.0, "speed": 165.0, "ai": "runner",
		"color": Color(0.95, 0.42, 0.42), "size": 14.0,
		"sprite": "imp",
	...
	"SENTRY": {
		...
		"color": Color(0.72, 0.32, 0.62), "size": 16.0,
		"sprite": "necromancer",
	...
	"LOBBER": {
		...
		"color": Color(0.6, 0.8, 0.45), "size": 15.0,
		"sprite": "orc_shaman",
	...
	"HOPPER": {
		...
		"color": Color(0.82, 0.8, 0.86), "size": 17.0,
		"sprite": "masked_orc",
	...
	"DRIFTER": {
		...
		"color": Color(0.5, 0.85, 0.95), "size": 13.0,
		"sprite": "angel",
	...
	"WARDEN": {
		...
		"color": Color(0.45, 0.65, 0.98), "size": 20.0,
		"sprite": "ogre", "tint": Color(0.66, 0.82, 1.1),
	...
	"DUMMY": {
		...
		"color": Color(0.6, 0.62, 0.68), "size": 18.0,
		"sprite": "skelet",
	...
	"ARBITER": {
		...
		"color": Color(0.98, 0.35, 0.55), "size": 34.0,
		"sprite": "big_demon",
